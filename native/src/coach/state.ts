...
// fields (participantId, role, preSubmittedAt) are owned by submit.ts; progress
...
import { readMarker } from '../../../skills/coach-checkin/scripts/submit.ts';
import { writeFileSync } from 'node:fs';
import { join } from 'node:path';

// The marker is an open-ended JSON record: identity fields owned by submit.ts and
// progress fields owned here coexist, and read-merge-write must preserve unknown
// keys from the other side. The index signature models that loose contract.
...
export function readState(): MarkerState {
  return readMarker() ?? {};
}

// Merge progress fields in WITHOUT touching identity fields. Read-merge-write so
// a concurrent identity write from submit.ts is preserved (last-write-wins per
// field across processes is acceptable for a single attendee on one machine).
export function writeProgress(progress: Partial<MarkerState>): MarkerState {
  const existing: MarkerState = readMarker() ?? {};
  const next: MarkerState = { ...existing, ...progress, updatedAt: new Date().toISOString() };
...
export function recordCheckpoint(block: number): MarkerState {
