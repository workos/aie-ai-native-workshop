...
A ~90-minute interactive workshop. **Not a talk** — every block is hands-on, one
...
| 0:30 | **Block 2 — Loops & goals** | [Goals as checklists + `/loop`](../curriculum/02-loops-and-goals.md); when to use which; parallel agents on worktrees. | — |
| 0:55 | **Block 3 — Verification gates** | [Lint/typecheck/test hook + adversarial Codex review](../curriculum/03-verification-gates.md); require the gate on the next problem. | — |
| 1:20 | **Block 4 — Scheduled tasks** | [Schedule the work they built](../curriculum/04-scheduled-tasks.md) → see it fire / get pinged → **closing check-in**. | Board migrates toil → leverage; hours-reclaimed reveal |
| 1:40 | **Close** | The board's before→after is the finale. Hand off the repo + the scheduled task they keep. | — |
